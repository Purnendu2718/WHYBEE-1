...
export default async function ParentAttendancePage({ searchParams }: { searchParams: { studentId?: string } }) {
  ...
  const { data: attendance, stats } = await getStudentAttendance(selectedStudent.id);

  return (
    <div className="space-y-6">
      <Navbar
        title={`Child Attendance (${selectedStudent.profile?.full_name || 'Student'})`}
        role="parent"
        extraRight={<ChildSwitcher students={linkedStudents} currentStudentId={selectedStudent.id} />}
      />

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <StatCard title="Attendance Rate" value={`${stats.pct}%`} icon={<Calendar className="w-5 h-5" />} color="blue" />
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs">
        <div className="p-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-800">Child Attendance Logs (Read Only)</h3>
          <span className="text-xs text-slate-500 font-medium">Roll No: {selectedStudent.roll_no}</span>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {attendance.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 transition">
                  ...
                  <td className="px-6 py-3.5 text-slate-500">{selectedStudent.class} - {selectedStudent.section}</td>
                </tr>
              ))}
            </tbody>
          </table>
